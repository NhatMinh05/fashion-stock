...
export default function AdminView() {
  // --- STATES CẤU HÌNH API ---
  const [apiKey, setApiKey] = useState("AIzaSyChih33uNHhj0HBmb6K93XmU7J8yvtDv0U");
  ...
  useEffect(() => {
    ...
    // Đọc xong thì cắm cờ báo hiệu
    setIsLoaded(true);
  }, []);

  // 2. Chỉ LƯU dữ liệu Công tắc khi người dùng GẠT NÚT (Sau khi trang đã load xong)
  useEffect(() => {
    if (isLoaded) {
  ...
  // Giả lập log chạy realtime khi ấn nút
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-[1600px] mx-auto space-y-6 pb-10">
      ...
      {/* HÀNG 1: STATUS CARDS (Giống hình mẫu) */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white/5 backdrop-blur-md p-6 rounded-2xl border border-white/10 shadow-xl relative overflow-hidden">
          ...
          <div className="space-y-5 text-sm">
            {/* Nhập Key */}
            <div>
              <label className="block text-slate-400 font-bold uppercase tracking-wider text-[10px] mb-2">Gemini API Key</label>
              <div className="relative">
                ...
                />
                <button onClick={() => setShowKey(!showKey)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-[#00F2FF]">
            ...
            <div>
              ...
              <select value={aiModel} onChange={(e) => { setAiModel(e.target.value); addLog(`Model switched to ${e.target.value}`); }} className="w-full bg-[#0A0817] border border-white/20 rounded-xl p-2.5 text-slate-300 focus:border-[#00F2FF] outline-none font-mono text-xs cursor-pointer appearance-none">
                ...
                <option value="gemini-pro-latest">gemini-pro-latest (Experimental)</option>
              </select>
            </div>
            ...
            <div className="pt-2">
              <div className="flex justify-between items-end mb-2">
                  <label className="block text-slate-400 font-bold uppercase tracking-wider text-[10px]">AI Temperature (Creativity)</label>
                  <span className="text-[#00F2FF] font-black text-sm">{temperature}</span>
              </div>
